...
from ..utils import DeformConv2d
...
class AEM(nn.Module):
    def __init__(self):
        ...
        self.conv_p = nn.Conv2d(256*4, 256, kernel_size=1)
        ...
        # self.Deconv1 = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=8, stride=8, padding=0)
        # self.Deconv2 = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=4, stride=4, padding=0)
    ...
    def forward(self, x):
        ...
        p5_1 = F.interpolate(p5, size=out_size, mode="nearest")
        pc = torch.cat((p2, p3_1, p4_1, p5_1), dim=1)
        pc = self.conv_p(pc)
        pc1 = self.conv_p1(pc)
        ...
        SFF_d4 = self.SSF(d4)
        SFF_d3 = self.SSF(d3)
        a5 = F.adaptive_max_pool2d(ac, output_size=p5.shape[-2:])
        a4 = F.adaptive_max_pool2d(ac, output_size=p4.shape[-2:]) * SFF_d5
...
class DIM(BaseModule):
    ...
    def __init__(self,
        ...
        super(DIM, self).__init__(init_cfg)
