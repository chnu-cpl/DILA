...
from mmcv.ops import batched_nms, DeformConv2d, ModulatedDeformConvG, ModulatedDeformConv2d
...
class RPNHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        if self.dcn_assign == False:
            self.rpn_conv = nn.Conv2d(self.in_channels, self.feat_channels,
    ...
    def forward_single(self, x):
        ...
        if self.dcn_assign == False:
            ...
            # reg_feat = self.rpn_conv[-2](reg_feat, sampling_loc)
            reg_feat, offsets_reg, mask_reg = self.rpn_conv[-1](reg_feat)

            # cls_feat = self.rpn_conv[0](cls_feat, sampling_loc)
            cls_feat = self.rpn_conv[1](cls_feat, offsets_reg, mask_reg)
            cls_feat = F.relu(cls_feat, inplace=True)
            reg_feat = F.relu(reg_feat, inplace=True)
